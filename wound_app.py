"""
AI 외상 응급도 판별 서비스 - 프로토타입
한양대학교 ERICA 인공지능프로그래밍 프로젝트

실행 방법:
    python3 wound_app.py       ← VSCode Code Runner / 터미널 모두 OK
    streamlit run wound_app.py ← 직접 streamlit 명령도 OK
"""
import sys
import os
import subprocess
...
# ─────────────────────────────────────────────
# 공공데이터포털 응급의료정보 API 키
#   로컬: .streamlit/secrets.toml 의 API_KEY 값 사용
#   Streamlit Cloud: 앱 설정 > Secrets 에 API_KEY 등록
# ─────────────────────────────────────────────
API_KEY = os.environ.get("API_KEY", "")
API_BASE = "https://apis.data.go.kr/B552657/ErmctInfoInqireService"

# 주요 도시 위경도 (위치 선택 드롭다운용)
CITIES = {
    "서울": (37.5665, 126.9780),
    "부산": (35.1796, 129.0756),
    "인천": (37.4563, 126.7052),
...
# API 호출 함수
# ─────────────────────────────────────────────
def fetch_nearby_hospitals(lat: float, lon: float, radius: int = 5000, rows: int = 10) -> list[dict]:
    """
    getEgytLcinfoInqire: 위경도 반경 내 응급의료기관 목록 조회
    """
    import requests
    ...
    try:
        resp = requests.get(
            f"{API_BASE}/getEgytLcinfoInqire",
            params=params,
            timeout=5,
        )
        data = resp.json()
...
def fetch_realtime_beds(hpid: str) -> dict | None:
    """
    getEmrrmRltmUsefulSckbdInfoInqire: 응급실 실시간 가용병상 조회
    """
    import requests
    ...
    try:
        resp = requests.get(
            f"{API_BASE}/getEmrrmRltmUsefulSckbdInfoInqire",
            params=params,
            timeout=5,
        )
        data = resp.json()
        ...
        if not items:
            return None
        item = items["item"]
        return item if isinstance(item, dict) else item[0]
    except Exception:
        return None
...
def main():
    ...
    # API 키: 환경변수 → Streamlit secrets 순으로 읽기
    global API_KEY
    if not API_KEY:
        try:
    ...
    LEVELS = {
        0: {
            "icon": "🟢", "label": "자가 처치 가능", "css": "level-0",
            "guide": (
                "• 상처 부위를 흐르는 물로 5분 이상 세척하세요.\n"
                "• 깨끗한 거즈나 밴드로 가볍게 덮어 보호하세요.\n"
                "• 24–48시간 내 붓기·발열·통증이 심해지면 병원을 방문하세요.\n"
                "• 파상풍 예방접종이 10년 이내인지 확인하세요."
            ),
        },
        1: {
            "icon": "🟡", "label": "가까운 병원 내원 권고", "css": "level-1",
            "guide": (
                "• 오늘 중 가까운 외과 또는 응급의원 방문을 권장합니다.\n"
                "• 이동 전 깨끗한 천으로 상처를 가볍게 압박하세요.\n"
                "• 봉합이 필요할 수 있으니 직접 테이프로 붙이지 마세요.\n"
                "• 혼자 이동이 어려우면 보호자와 동행하세요."
            ),
        },
        2: {
            "icon": "🔴", "label": "즉시 응급실 방문 필요", "css": "level-2",
            "guide": (
                "• 지금 즉시 119에 신고하거나 응급실로 이동하세요.\n"
                "• 출혈이 있으면 깨끗한 천으로 강하게 압박하고 심장보다 높게 올리세요.\n"
                "• 혼자 운전하지 마세요. 구급대 또는 보호자를 이용하세요.\n"
                "• 응급의료정보센터 ☎ 1339"
            ),
        },
    }
    ...
    if uploaded is not None:
        ...
        with st.spinner("AI가 사진을 분석하고 있습니다..."):
            level, score, detected, primary_type, detail = analyze_wound(image)
        ...
        # 3구간 정규화: 자가(0-33%) / 병원(33-66%) / 응급(66-100%)
        # 원점수 0.00-0.10 → 0-33%, 0.10-0.25 → 33-66%, 0.25+ → 66-100%
        if score < 0.10:
            bar_pct = (score / 0.10) * 33
        elif score < 0.25:
            bar_pct = 33 + ((score - 0.10) / 0.15) * 33
        else:
            bar_pct = 66 + min((score - 0.25) / 0.75, 1.0) * 34
        bar_pct = round(bar_pct, 1)

        st.markdown(
            f"""
            <div style="margin:16px 0 6px; font-weight:600; font-size:0.9rem; color:#374151;">
                응급도 게이지
            </div>
            <div style="position:relative; height:18px; border-radius:10px; overflow:visible;
                ...
                <div style="position:absolute; left:{bar_pct}%; top:50%; transform:translate(-50%,-50%);
                            ...
                            border:3px solid #374151; box-shadow:0 2px 6px rgba(0,0,0,0.25);
                            z-index:10;">
                </div>
            </div>
        ...
        WOUND_GUIDES = {
            ...
            "부종·염좌 의심": (
                "• RICE 원칙을 따르세요: 휴식·냉찜질·압박·거상(높이 올리기).\n"
        ...
        guide_text = WOUND_GUIDES.get(primary_type, meta["guide"])
        guide_html = "<br>".join(guide_text.strip().split("\n"))
        guide_note = f" (복합 외상 — 가장 심각한 '{primary_type}' 기준)" if len(detected) > 1 else f" — {primary_type}"
        st.markdown(
        ...
        with st.expander("🔬 세부 분석 수치 보기"):
            ...
            for col, (k, v) in zip(cols, detail.items()):
                col.metric(k, v)

        # ── 주변 병원/응급실 지도 (🟡·🔴 결과에서만 표시) ──
        if level >= 1:
            import pandas as pd

            st.markdown("---")
            st.markdown(
                f"### {'🚨 가까운 응급실 찾기' if level == 2 else '🏥 가까운 병원 찾기'}"
            )
            ...
            # 버튼 클릭 → 원거리까지 충분히 fetch 후 session_state 저장
            if search_btn:
                lat, lon = CITIES[city]
                with st.spinner("주변 응급의료기관을 검색 중입니다..."):
                    raw = fetch_nearby_hospitals(lat, lon, rows=50)
                st.session_state.hospitals_raw  = raw
                st.session_state.hospital_city  = city
                st.session_state.hospital_radius = radius_km

            # 반경은 매 렌더마다 현재 슬라이더 값으로 클라이언트 필터링
            raw_list  = st.session_state.hospitals_raw
            hospitals = [
                h for h in raw_list
            ...
            if hospitals:
                ...
                # ── 지도 (CartoDB Positron 타일) ───────
                # 반경 5km마다 줌 1단계씩 축소 (1~5→12, 6~10→11, 11~15→10, 16~20→9)
                ...
                rows_data = [
                    {
                        "기관명":    h.get("dutyName", "-"),
                        "분류":      h.get("dutyDivName", "-"),
                        "거리(km)":  h.get("distance", "-"),
                        "전화번호":  h.get("dutyTel1", "-"),
                        "주소":      h.get("dutyAddr", "-"),
                    }
                    for h in hospitals
